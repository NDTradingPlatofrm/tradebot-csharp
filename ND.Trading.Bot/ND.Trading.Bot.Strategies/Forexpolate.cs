...
namespace ND.Trading.Bot.Strategies
{
    public class Forexpolate : Strategy
    {
        ...
        private ForexData FxStock { get; set; }
        ...
        private void RunStrategy()
        {
            ...
            {
                for (var iPos = this.CurrentAccount.PositionList.Count - 1; iPos >= 0; --iPos)
                {
                    this.StockObject = this.CurrentAccount.PositionList[iPos];
        ...
        protected bool IsEnoughPositionToTrade()
        {
            ...
            if (this.CurrentAccount.PositionList.Count >= expectedPositionLevel)
                rtnVal = true;
            return rtnVal;
        }
        ...
        {
            bool rtnVal = false;
            ForexPosition fxPos = (ForexPosition)this.CurrentAccount.PositionList.Find(x => x.Symbol == symbol);
            if (fxPos != null)
                rtnVal = true;
        ...
        private bool IsReadyToAddMore()
        {
            ...
            if (posConfig.SellMode == Constants.TradeMode.FIXED)
            {
                //Need short buy change
                if (type == "Long")
                    this.CurrentPrice = ExchangeObject.GetCurrentPrice(this.StockObject.Symbol, Constants.OrderAction.BUY);   //Service call 6 - every for loop
                else if (type == "Short")
                    this.CurrentPrice = ExchangeObject.GetCurrentPrice(this.StockObject.Symbol, Constants.OrderAction.SELL);   //Service call 6 - every for loop
        ...
        private void AddMoreStocks()
        {
            ...
            if (((ForexPosition)this.StockObject).TradeList.Count < this.posConfig.ExtrapolateLevel)
            {
                ...
                decimal margin = this.CurrentAccount.MarginAvailable;
        ...
        {
            this.FxStock = (ForexData)this.StockObject.TradeList[index];
